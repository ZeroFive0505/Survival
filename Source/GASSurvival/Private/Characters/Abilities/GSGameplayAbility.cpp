...
FGameplayAbilityTargetDataHandle UGSGameplayAbility::MakeGameplayAbilityTargetDataHandleFromActorArray(
...
FGameplayAbilityTargetDataHandle UGSGameplayAbility::MakeGameplayAbilityTargetDataHandleFromHitResults(
...
FGSGameplayEffectContainerSpec UGSGameplayAbility::MakeEffectContainerSpecFromContainer(
	const FGSGameplayEffectContainer& Container, const FGameplayEventData& EventData, int32 OverrideGameplayLevel)
{
	FGSGameplayEffectContainerSpec ReturnSpec;
	const AActor* OwningActor = GetOwningActorFromActorInfo();
	AActor* AvatarActor = GetAvatarActorFromActorInfo();
	AGSCharacter* AvatarCharacter = Cast<AGSCharacter>(AvatarActor);
	const UGSAbilitySystemComponent* OwningASC = UGSAbilitySystemComponent::GetAbilitySystemComponentFromActor(OwningActor);
...
FGSGameplayEffectContainerSpec UGSGameplayAbility::MakeEffectContainerSpec(FGameplayTag ContainerTag,
                                                                           const FGameplayEventData& EventData,
                                                                           int32 OverrideGameplayLevel)
{
	const FGSGameplayEffectContainer* FoundContainer = EffectContainerMap.Find(ContainerTag);
...
TArray<FActiveGameplayEffectHandle> UGSGameplayAbility::ApplyEffectContainerSpec(
...
UObject* UGSGameplayAbility::BP_GetSourceObject(FGameplayAbilitySpecHandle Handle,
                                                const FGameplayAbilityActorInfo& ActorInfo) const
{
	return GetSourceObject(Handle, &ActorInfo);
}

bool UGSGameplayAbility::BatchRPCTryActivateAbility(FGameplayAbilitySpecHandle InAbilityHandle,
                                                    bool EndAbilityImmediately)
{
	UGSAbilitySystemComponent* ASC = Cast<UGSAbilitySystemComponent>(GetAbilitySystemComponentFromActorInfo());
...
bool UGSGameplayAbility::CanActivateAbility(const FGameplayAbilitySpecHandle Handle,
                                            ...
                                            FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	{
		const AGSSurvivor* Survivor = Cast<AGSSurvivor>(ActorInfo->AvatarActor);
		if (Survivor && Survivor->GetCurrentWeapon() && static_cast<UObject*>(Survivor->GetCurrentWeapon()) ==
...
bool UGSGameplayAbility::CheckCost(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo,
                                   FGameplayTagContainer* OptionalRelevantTags) const
{
	return Super::CheckCost(Handle, ActorInfo, OptionalRelevantTags) && GSCheckCost(Handle, *ActorInfo);
}

bool UGSGameplayAbility::GSCheckCost_Implementation(const FGameplayAbilitySpecHandle Handle,
                                                    const FGameplayAbilityActorInfo& ActorInfo) const
{
	return true;
}

void UGSGameplayAbility::ApplyCost(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo,
...
void UGSGameplayAbility::ResetHUDReticle()
{
	...
	if (PC)
	{
		const AGSSurvivor* Character = Cast<AGSSurvivor>(CurrentActorInfo->AvatarActor);
		if (Character && Character->GetCurrentWeapon())
		{
			PC->SetHUDReticle(Character->GetCurrentWeapon()->GetPrimaryHUDReticleClass());
		}
		else
		{
...
bool UGSGameplayAbility::FindAbilityMeshMontage(const USkeletalMeshComponent* InMesh, FAbilityMeshMontage& InAbilityMontage)
{
...
void UGSGameplayAbility::MontageJumpToSectionForMesh(USkeletalMeshComponent* InMesh, FName SectionName)
{
	...
	UGSAbilitySystemComponent* const AbilitySystemComponent = Cast<UGSAbilitySystemComponent>(
...
void UGSGameplayAbility::MontageSetNextSectionNameForMesh(USkeletalMeshComponent* InMesh, FName FromSectionName,
                                                          FName ToSectionName)
{
	check(CurrentActorInfo);
